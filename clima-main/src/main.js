...
const selectedLocationWeather = {
	...
	displayWeatherCondition: function (data) {
		...
		switch (weatherType) {
			...
			case 'Drizzle':
			case 'Clouds':
				this.conditionMsg.innerHTML = `<i class="fa-solid fa-umbrella"></i> Paraguas `;
				break;
			case 'Thunderstorm':
			case 'Tornado':
				this.conditionMsg.innerHTML = `<i class="fa-solid fa-cloud-bolt"></i> Permanezca en el Interior`;
				break;
			case 'Snow':
				this.conditionMsg.innerHTML = `<i class="fa-solid fa-snowflake"></i> Abríguese`;
				break;
			case 'Clear':
				this.conditionMsg.innerHTML = `<i class="fa-solid fa-circle-check"></i> Condiciones Ideales`;
				break;
			case 'Mist':
			case 'Fog':
			case 'Haze':
				this.conditionMsg.innerHTML = `<i class="fa-solid fa-triangle-exclamation"></i> Poca visibilidad`;
				break;
			default:
				this.conditionMsg.innerHTML = `<i class="fa-solid fa-triangle-exclamation"></i> Mala calidad del aire`;
		}
	},
};
		...
		forecastData.forEach((day, index) => {
			...
			<div class="daily m-2 m-md-0">
				<p>${timeManager.formatDay(day.dt)}</p>
					...
					/>
				<p>
					<span class="temps">${Math.round(day.temp.max)}</span>°<span class="fahrenheit">${weatherService.units === 'metric' ? 'C' : 'F'
					}
					</span><br />
					<span class="daily-low">
						<span class="forecast-low temps">${Math.round(day.temp.min)}</span>°<span class="fahrenheit">${weatherService.units === 'metric' ? 'C' : 'F'
					}
					</span>
					</span>
				</p>
...
const OPENWEATHER_KEY = 'eca924cc4dc7e450672b550f03e1fc73';
